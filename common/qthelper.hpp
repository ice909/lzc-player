    ...
    namespace qt
    {
        ...
        private:
            ...
            char *dup_qstring(const QString &s)
            {
                ...
                auto *result = new char[bytes.size() + 1];
                if (result)
                {
                    std::memcpy(result, bytes.data(), bytes.size() + 1);
                }
                return result;
            }
            ...
            void set(mpv_node *dst, const QVariant &src)
            {
                ...
                {
                    const QVariantList qlist = src.toList();
                ...
                {
                    const QVariantMap qmap = src.toMap();
        ...
        {
            mpv_node node;
            const int err = mpv_get_property(ctx, name.toUtf8().data(), MPV_FORMAT_NODE, &node);
            if (err < 0)
            {
